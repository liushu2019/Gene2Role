...
def exec_signeds2v(args):
    ...
    if args.output is None:
        args.output = os.path.join(os.path.dirname(args.input), args.project)
...
def main():
    ...
    parser.add_argument('--input', nargs='?', default='',
                        help='[SignedS2V] Input graph path')
    ...
    if args.TaskMode == 1: #run SignedS2V
        ...
        if args.CellType == 2: #multi cell
            ...
            index_tracker = pd.read_csv(os.path.join(output_directory, 'index_tracker.tsv'), sep='\t')
            list_input_file = []
